...
import { ensureSearchArgs, uploadFiles, buildSearchJsonBody, postSearch, computeModelPreference, buildModelPrefMap, cryptoRandomUuid } from './search_helpers';
...
export class PerplexityClient {
  ...
  private signin_regex = /"(https:\/\/www\.perplexity\.ai\/api\/auth\/callback\/email\?callbackUrl=.*?)"/;
  ...
  const jsonBody = buildSearchJsonBody(this, query, mode, model as any, uploaded_files, follow_up, incognito, language, sources);
  const res = await postSearch(this, jsonBody);
    ...
    const self = this;
    async function* wrapper(): AsyncGenerator<PerplexityChunk, PerplexityResponse, void> {
      ...
      for await (const chunk of self.sseStream(res)) {
        ...
        if (chunk.text && typeof chunk.text === 'string') chunk.text = [chunk.text];
        collected.push(chunk as PerplexityChunk);
        yield chunk as PerplexityChunk;
        if (chunk && (chunk.final === true || chunk.final_sse_message === true)) break;
      }
        ...
        const pushUnique = (targetKey: string, value: any) => {
          ...
          if (Array.isArray(value)) {
            for (const item of value) {
              if (!agg[targetKey].some((x: any) => JSON.stringify(x) === JSON.stringify(item))) agg[targetKey].push(item);
            }
          } else {
            if (!agg[targetKey].some((x: any) => JSON.stringify(x) === JSON.stringify(value))) agg[targetKey].push(value);
          }
        };
        ...
        if (Array.isArray(agg.blocks)) {
          ...
          for (let i = 0; i < agg.blocks.length; i++) {
            ...
            if (b && b.intended_usage === 'ask_text' && b.markdown_block) {
              if (firstAskTextIndex === null) firstAskTextIndex = mergedBlocks.length;
  ...
  private sizeOf(file: any): number {
    ...
    if (typeof Blob !== 'undefined' && file instanceof Blob) return (file as Blob).size;
    if (typeof ArrayBuffer !== 'undefined' && file instanceof ArrayBuffer) return file.byteLength;
  ...
  private async *sseStream(res: Response): AsyncGenerator<PerplexityChunk, void, void> {
    if (!res.body) return;
        ...
        if (content.startsWith('event: message\r\n')) {
          ...
          try {
            ...
            if (typeof textVal === 'string') {
              try { parsed.text = JSON.parse(textVal); } catch (e) { parsed.text = textVal; }
            }
  ...
  async createAccount(emailnatorCookies: Record<string,string>){
    ...
    if (!initResp.ok) throw new Error('signin request failed');

    // wait for email
    const new_msgs = await en.reload({ wait: true, wait_for: (m:any)=> m.subject === 'Sign in to Perplexity', timeout: 20 });
    if (!new_msgs) throw new Error('no signin email');
    ...
    if (!m) throw new Error('signin link not found');
    const link = m[1];
  ...
  async getModels(): Promise<any> {
      ...
      try {
        ...
        if (resp.ok) {
          ...
          if (ct.includes('application/json')) {
            try { return await resp.json(); } catch (e) { /* ignore parse error */ }
          }
        }
      } catch (e) {
        // ignore and try next
    ...
    try {
      const cookieVal = (this.cookies && (this.cookies['pplx.search-models-v4'] || this.cookies['pplx.search-models-v3'])) as any;
      if (cookieVal) {
        try {
    ...
    try {
      ...
      if (s.ok) {
        try {
          ...
          if (body && (body.search_models || body['pplx.search-models-v4'] || body.user)) return body;
        } catch (e) { }
      }
    } catch (e) { }
