...
export class LabsClient {
  ...
  async init(): Promise<this> {
    ...
    const resp = await fetch(
      `${this.base}/socket.io/?EIO=4&transport=polling&t=${this.timestamp}`
    );
    ...
    await fetch(
      `${this.base}/socket.io/?EIO=4&transport=polling&t=${this.timestamp}&sid=${this.sid}`,
      {
        method: "POST",
  ...
  async ask(
    ...
    model = "r1-1776",
    stream = false
  ): Promise<any | AsyncGenerator<any, void, void>> {
    ...
    const payload = [
      "perplexity_labs",
      { messages: this.history, model, source: "default", version: "2.18" },
    ];
