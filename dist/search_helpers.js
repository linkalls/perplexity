...
export function ensureSearchArgs(self, mode, sources, files) {
    ...
    if ((mode === 'pro' || mode === 'reasoning' || mode === 'deep research') && self.copilot <= 0)
        throw new Error('No remaining pro queries.');
    if (files && Object.keys(files).length && self.file_upload - Object.keys(files).length < 0)
    ...
    for await (const chunk of stream) {
        try {
            ...
            if (chunk.blocks && Array.isArray(chunk.blocks)) {
                for (const b of chunk.blocks) {
                    if (!b || typeof b !== 'object')
                        continue;
                    if (b.intended_usage === 'ask_text' && b.markdown_block) {
                        ...
                        if (md.answer && typeof md.answer === 'string') {
                            ...
                            outPieces.push(chunks.filter((x) => x != null).map((x) => typeof x === 'string' ? x : JSON.stringify(x)).join(''));
                        }
                    }
                }
            }
            if (outPieces.length > 0) {
                ...
                if (normalized)
                    yield { text: normalized, backend_uuid: backend };
            }
            else if (backend) {
