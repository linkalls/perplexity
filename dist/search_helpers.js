 ...
 */
export function ensureSearchArgs(self, mode, sources, files) {
    if (!["auto", "pro", "reasoning", "deep research"].includes(mode))
        throw new Error("Invalid search mode.");
    if (!sources.every((s) => ["web", "scholar", "social"].includes(s)))
    ...
    if (files && Object.keys(files).length)
        self.file_upload = Math.max(0, self.file_upload - Object.keys(files).length);
}
/**
 ...
 */
export async function uploadFiles(self, files) {
    ...
    for (const [filename, file] of Object.entries(files || {})) {
        ...
        try {
            ...
            if (upjson && upjson.secure_url) {
                if (file_upload_info.s3_object_url &&
 ...
 */
export function buildModelPrefMap() {
 ...
 */
export function computeModelPreference(mode, model) {
    ...
    // direct key match
    if (Object.prototype.hasOwnProperty.call(byMode, model))
        return byMode[model];
 ...
 * input.
 */
export function parseCookieEnv(raw) {
    const out = {};
    const s = (raw ?? "").trim();
    if (!s)
        return out;
    const tryHeaderParse = (hdr) => Object.fromEntries(hdr
        .split(";")
        .map((p) => p.trim())
        .filter(Boolean)
        .map((p) => {
    ...
    try {
        ...
        if (j && typeof j === "object") {
            if (typeof j.cookie === "string")
                return tryHeaderParse(j.cookie);
    ...
    if (/^\s*\{.*'/.test(s)) {
        try {
            ...
            if (j2 && typeof j2 === "object") {
                if (typeof j2.cookie === "string")
                    return tryHeaderParse(j2.cookie);
 ...
 */
export function buildSearchJsonBody(self, query, mode, model, uploaded_files, follow_up, incognito, language, sources) {
 ...
 */
export async function postSearch(self, jsonBody) {
 ...
 */
export async function* extractStreamEntries(stream) {
    ...
    for await (const chunk of stream) {
        try {
            const backend = chunk.backend_uuid;
            ...
            if (chunk.text) {
                ...
                for (const t of arr) {
                    ...
                    if (typeof t === "string")
                        ...
                        outPieces.push(JSON.stringify(t));
                }
            }
                ...
                for (const b of chunk.blocks) {
                    ...
                    if (b.intended_usage === "ask_text" &&
                        ...
                        try {
                            ...
                            if (b.web_result_block &&
                                Array.isArray(b.web_result_block.web_results)) {
                                for (const r of b.web_result_block.web_results) {
                            ...
                            if (b.plan_block &&
                                Array.isArray(b.plan_block.goals)) {
            ...
            if (outPieces.length > 0) {
                ...
                if (mergedRaw)
                    yield { text: mergedRaw, backend_uuid: backend };
            }
            else if (backend) {
 ...
 */
export async function* extractStreamAnswers(stream) {
 ...
 */
export async function* extractStreamBackend(stream) {
    const seen = new Set();
    for await (const e of extractStreamEntries(stream)) {
 ...
 */
export function removeNumericCitations(text) {
    if (!text)
        return text;
