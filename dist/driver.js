...
export class Driver {
    ...
    perplexity_headers = null;
    emailnator_headers = null;
    ...
    async initBrowser(chrome_data_dir) {
        ...
        console.log(`🚀 Driver initializing using ${usingPatchright ? "patchright" : "playwright"}`);
        console.log("🔧 Getting chromium implementation...");
        const impl = playwright.chromium;
    ...
    async accountCreator() {
        ...
        const Emailnator = EmailnatorModule
        ...
        while (true) {
            if (!this.newAccountLink) {
                ...
                try {
                    ...
                    if (!Emailnator)
                        throw new Error("Emailnator module not available (install or implement src/emailnator.ts)");
                    ...
                    const resp = await fetch("https://www.perplexity.ai/api/auth/signin/email", {
                        method: "POST",
                        headers,
                        body,
                    }).catch((e) => {
                    ...
                    if (resp && resp.ok) {
                        ...
                        const newMsgs = await en
                            .reload({
                            wait_for: (m) => m && m.subject && /perplexity/i.test(m.subject),
                            timeout: 20,
                        })
                            .catch(() => null);
                        if (newMsgs) {
                    ...
                    console.warn("accountCreator error, will attempt to renew emailnator cookies:", e);
    ...
    async interceptRequest(route, request) {
        try {
            ...
            if (url === "https://www.perplexity.ai/") {
                ...
                if (!this.perplexity_cookies &&
                    fetchedResponse &&
                    (await fetchedResponse.text()).includes("What do you want to know?") &&
            ...
            if (url === "https://www.emailnator.com/") {
                const cookies = this.parseCookies(request.headers()["cookie"]);
                if (!this.emailnator_cookies &&
                    fetchedResponse &&
                    (await fetchedResponse.text()).includes("Temporary Disposable Gmail") &&
                    cookies["XSRF-TOKEN"]) {
    ...
    async run(chrome_data_dir, port) {
        ...
        try {
            ...
            throw new Error("Playwright or Patchright is not installed. Install patchright (preferred) or playwright and provide a Chrome user data dir.");
        }
        // minimal launch/connect logic
        try {
            // log which implementation we're using
            // eslint-disable-next-line no-console
            console.log(`Driver starting using ${usingPatchright ? "patchright" : "playwright"} (port: ${port ?? "none"})`);
            if (port) {
     ...
     */
    async performInteractiveSignin(opts = {}) {
        ...
        if (!playwright) {
            console.warn("performInteractiveSignin: neither patchright nor playwright is available");
            return null;
        }
        const impl = playwright.chromium;
        try {
